...
TEST(UU_UD, PS1) {
    ...
    ASSERT_NEAR(m2 / 3.745797690449368546018128e-12, 1.0, 1e-6);

    // u u > mu+ vm s u
    m2 = me.Calculate(ps, perm1, param) * Vus2;
    ASSERT_NEAR(m2 / 2.001686695876556839413147e-13, 1.0, 1e-6);

    // u u > mu+ vm b u
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 4.502059593630847472634132e-17, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 6.633476694455681702972452e-15, 1.0, 1e-6);
}
