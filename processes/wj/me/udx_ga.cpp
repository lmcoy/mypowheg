...
// Process: u d~ > mu+ vm g a WEIGHTED<=7 @1
// Process: c s~ > mu+ vm g a WEIGHTED<=7 @1

//--------------------------------------------------------------------------
// Evaluate |M|^2, part independent of incoming flavour.

double UDX_GA::Calculate(const Phasespace::Phasespace &ps, int perm[],
...
double UDX_GA::calculate_wavefunctions(const int perm[], const int hel[],
                                       const Parameters_sm &pars,
                                       double momenta[][4]) {

    constexpr std::complex<double> ZERO(0.0, 0.0);

    constexpr int nwavefuncs = 18;
    ...
    FFV1_2(w[0], w[4], pars.GC_11, ZERO, w[6]);
    FFV2_3(w[2], w[3], pars.GC_100, pars.MuW, w[7]);
    FFV2_3(w[6], w[1], pars.GC_100, pars.MuW, w[8]);
    FFV1_2(w[6], w[5], pars.GC_2, ZERO, w[9]);
    FFV1_1(w[1], w[5], pars.GC_1, ZERO, w[10]);
    FFV1_2(w[2], w[5], pars.GC_3, ZERO, w[11]);
    FFV1_1(w[1], w[4], pars.GC_11, ZERO, w[12]);
    FFV1_2(w[0], w[5], pars.GC_2, ZERO, w[13]);
    FFV2_3(w[0], w[12], pars.GC_100, pars.MuW, w[14]);
    FFV1_1(w[12], w[5], pars.GC_1, ZERO, w[15]);
    FFV1_2(w[13], w[4], pars.GC_11, ZERO, w[16]);
    FFV1_1(w[10], w[4], pars.GC_11, ZERO, w[17]);

    // Calculate all amplitudes
    // Amplitude(s) for diagram number 0

    constexpr int namplitudes = 10;
    ...
    FFV2_0(w[0], w[15], w[7], pars.GC_100, amp[6]);
    FFV2_0(w[11], w[3], w[14], pars.GC_100, amp[7]);
    FFV2_0(w[16], w[1], w[7], pars.GC_100, amp[8]);
    FFV2_0(w[0], w[17], w[7], pars.GC_100, amp[9]);
