...
TEST(GU_GD, PS1) {
    ...
    ASSERT_NEAR(m2 / 4.499146647236921539622368e-11, 1.0, 1e-6);

    // u g > mu+ vm g s
    m2 = me.Calculate(ps, perm1, param) * Vus2;
    ASSERT_NEAR(m2 / 2.404262784809227198395357e-12, 1.0, 1e-6);

    // u g > mu+ vm g b
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 5.407506758304152752314293e-16, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 7.967591123642392822186691e-14, 1.0, 1e-6);

    int perm2[] = { 0, 1, 2, 3, 4, 5 };
    // g u > mu+ vm g d
    m2 = me.Calculate(ps, perm2, param) * Vud2;
    ASSERT_NEAR(m2 / 5.311588759278755778029998e-11, 1.0, 1e-6);

    // g u > mu+ vm g s
    m2 = me.Calculate(ps, perm2, param) * Vus2;
    ASSERT_NEAR(m2 / 2.838417189621434663159348e-12, 1.0, 1e-6);

    // g u > mu+ vm g b
    m2 = me.Calculate(ps, perm2, param) * Vub2;
    ASSERT_NEAR(m2 / 6.383977755153118357513606e-16, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 9.406354308735101443450942e-14, 1.0, 1e-6);
}
