...
TEST(GDX_GUX, PS1) {
    ...
    int perm1[] = { 1, 0, 2, 3, 5, 4 };
    double m2 = 0.0;

    // b~ g > mu+ vm c~ g
    m2 = me.Calculate(ps, perm1, param) * Vcb2;
    ASSERT_NEAR(m2 / 9.890271263827478414648810e-15, 1.0, 1e-6);

    // b~ g > mu+ vm u~ g
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 6.712406280727824839468208e-17, 1.0, 1e-6);

    // s~ g > mu+ vm c~ g
    m2 = me.Calculate(ps, perm1, param) * Vcs2;
    ASSERT_NEAR(m2 / 5.584847428336770764736195e-12, 1.0, 1e-6);

    // s~ g > mu+ vm u~ g
    m2 = me.Calculate(ps, perm1, param) * Vus2;
    ASSERT_NEAR(m2 / 2.984441691633647015129125e-13, 1.0, 1e-6);
    ...
    int perm2[] = { 0, 1, 2, 3, 5, 4 };
    // g b~ > mu+ vm c~ g
    m2 = me.Calculate(ps, perm2, param) * Vcb2;
    ASSERT_NEAR(m2 / 9.989847140921989991551160e-15, 1.0, 1e-6);

    // g b~ > mu+ vm u~ g
    m2 = me.Calculate(ps, perm2, param) * Vub2;
    ASSERT_NEAR(m2 / 6.779987212027731469494520e-17, 1.0, 1e-6);

    // g s~ > mu+ vm c~ g
    m2 = me.Calculate(ps, perm2, param) * Vcs2;
    ASSERT_NEAR(m2 / 5.641076025741331703584296e-12, 1.0, 1e-6);

    // g s~ > mu+ vm u~ g
    m2 = me.Calculate(ps, perm2, param) * Vus2;
    ASSERT_NEAR(m2 / 3.014489239486934558350420e-13, 1.0, 1e-6);
