...
TEST(UUX_DUX, PS1) {
    ...
    ASSERT_NEAR(m2 / 2.665407224153506722188635e-12, 1.0, 1e-6);

    // u u~ > mu+ vm u~ s
    m2 = me.Calculate(ps, perm1, param) * Vus2;
    ASSERT_NEAR(m2 / 1.424345525463036891395045e-13, 1.0, 1e-6);

    // u u~ > mu+ vm u~ b
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 3.203542517800444481502366e-17, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 4.720200652517082727789249e-15, 1.0, 1e-6);

    int perm2[] = { 1, 0, 2, 3, 5, 4 };
    // c~ c > mu+ vm c~ d
    m2 = me.Calculate(ps, perm2, param) * Vcd2;
    ASSERT_NEAR(m2 / 3.270682231177893649789909e-13, 1.0, 1e-6);

    // c~ c > mu+ vm c~ s
    m2 = me.Calculate(ps, perm2, param) * Vcs2;
    ASSERT_NEAR(m2 / 6.120495266805462278323053e-12, 1.0, 1e-6);

    // c~ c > mu+ vm c~ b
    m2 = me.Calculate(ps, perm2, param) * Vcb2;
    ASSERT_NEAR(m2 / 1.083885625067212868041107e-14, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 7.356199322763767402685860e-17, 1.0, 1e-6);
}
