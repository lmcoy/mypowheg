...
TEST(UXDX_UXUX, PS1) {
    ...
    ASSERT_NEAR(m2 / 1.641316822876478734635893e-15, 1.0, 1e-6);

    // b~ u~ > mu+ vm u~ u~
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 1.113941676284895730619119e-17, 1.0, 1e-6);

    // s~ c~ > mu+ vm c~ c~
    m2 = me.Calculate(ps, perm1, param) * Vcs2;
    ASSERT_NEAR(m2 / 9.268202855924698563817404e-13, 1.0, 1e-6);

    // s~ u~ > mu+ vm u~ u~
    m2 = me.Calculate(ps, perm1, param) * Vus2;
    ASSERT_NEAR(m2 / 4.952760368957345237904099e-14, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 4.227172346792057673674619e-15, 1.0, 1e-6);

    // c~ s~ > mu+ vm c~ c~
    m2 = me.Calculate(ps, perm2, param) * Vcs2;
    ASSERT_NEAR(m2 / 2.387003549281996986059774e-12, 1.0, 1e-6);

    // c~ d~ > mu+ vm c~ c~
    m2 = me.Calculate(ps, perm2, param) * Vcd2;
    ASSERT_NEAR(m2 / 1.275571625181576509661757e-13, 1.0, 1e-6);

    // u~ b~ > mu+ vm u~ u~
    m2 = me.Calculate(ps, perm2, param) * Vub2;
    ASSERT_NEAR(m2 / 2.868930229861584706939666e-17, 1.0, 1e-6);
