...
TEST(DDX_DUX, PS1) {
    ...
    ASSERT_NEAR(m2 / 6.797147226257658030269728e-14, 1.0, 1e-6);

    // d d~ > mu+ vm u~ d
    m2 = me.Calculate(ps, perm1, param) * Vud2;
    ASSERT_NEAR(m2 / 1.271964210693360168384118e-12, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 2.252536213936291678514309e-15, 1.0, 1e-6);

    // b b~ > mu+ vm u~ b
    m2 = me.Calculate(ps, perm1, param) * Vub2;
    ASSERT_NEAR(m2 / 1.528768809940760941524733e-17, 1.0, 1e-6);

    int perm2[] = { 1, 0, 2, 3, 5, 4 };
    // b~ b > mu+ vm c~ b
    m2 = me.Calculate(ps, perm2, param) * Vcb2;
    ASSERT_NEAR(m2 / 2.637807949870813905927815e-15, 1.0, 1e-6);

    // b~ b > mu+ vm u~ b
    m2 = me.Calculate(ps, perm2, param) * Vub2;
    ASSERT_NEAR(m2 / 1.790248030387641501722933e-17, 1.0, 1e-6);

    // s~ s > mu+ vm c~ s
    m2 = me.Calculate(ps, perm2, param) * Vcs2;
    ASSERT_NEAR(m2 / 1.489519807122175509484255e-12, 1.0, 1e-6);

    // s~ s > mu+ vm u~ s
    m2 = me.Calculate(ps, perm2, param) * Vus2;
    ASSERT_NEAR(m2 / 7.959725077419730570475962e-14, 1.0, 1e-6);
