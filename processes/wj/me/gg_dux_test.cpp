...
TEST(GG_DUX, PS1) {
    ...
    ASSERT_NEAR(m2 / 5.526955115099561300236583e-14, 1.0, 1e-6);

    // g g > mu+ vm c~ s
    m2 = me.Calculate(ps, perm1, param) * Vcs2;
    ASSERT_NEAR(m2 / 1.034270535344257566503503e-12, 1.0, 1e-6);

    // g g > mu+ vm c~ b
    m2 = me.Calculate(ps, perm1, param) * Vcb2;
    ASSERT_NEAR(m2 / 1.831601719831610252622055e-15, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 1.243085711203405661868060e-17, 1.0, 1e-6);
}
