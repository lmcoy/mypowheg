...
TEST(UD_DD, PS1) {
    ...
    ASSERT_NEAR(m2 / 3.442370274749206915899339e-12, 1.0, 1e-6);

    // d c > mu+ vm d d
    m2 = me.Calculate(ps, perm1, param) * Vcd2;
    ASSERT_NEAR(m2 / 1.839540560029494467131210e-13, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 4.137371369462593134754921e-17, 1.0, 1e-6);

    // b c > mu+ vm b b
    m2 = me.Calculate(ps, perm1, param) * Vcb2;
    ASSERT_NEAR(m2 / 6.096133555065667217199592e-15, 1.0, 1e-6);

    int perm2[] = { 0, 1, 2, 3, 4, 5 };
    // u d > mu+ vm d d
    m2 = me.Calculate(ps, perm2, param) * Vud2;
    ASSERT_NEAR(m2 / 2.430392034027436892051771e-12, 1.0, 1e-6);

    // u s > mu+ vm s s
    m2 = me.Calculate(ps, perm2, param) * Vus2;
    ASSERT_NEAR(m2 / 1.298757648519310320358916e-13, 1.0, 1e-6);

    // u b > mu+ vm b b
    m2 = me.Calculate(ps, perm2, param) * Vub2;
    ASSERT_NEAR(m2 / 2.921078679976592050399505e-17, 1.0, 1e-6);
    ...
    ASSERT_NEAR(m2 / 4.304009519045237294434438e-15, 1.0, 1e-6);
}
