...
void Phasespaces::Generate(const Phasespace::Phasespace &ps, int jfks,
                              double xi, double y, double phi) {

    double xi_soft = 1e-10;
    double y1_coll = 0.999999;
    double y2_coll = -y1_coll;
    Phasespace::GenRealPhasespace(&Real, &ps, jfks, xi, y, phi);
    Phasespace::GenRealPhasespace(&Soft, &ps, jfks, xi_soft, y, phi);
    Phasespace::GenRealPhasespace(&Collinear1, &ps, jfks, xi, y1_coll, phi);
    Phasespace::GenRealPhasespace(&Collinear2, &ps, jfks, xi, y2_coll, phi);
    Born = ps;
}
